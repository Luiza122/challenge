...
export const clients = leads.map((lead, index) => ({
  ...lead,
...
export function predict(payload) {
  const region = String(payload.region || '').toLowerCase();
  const channel = String(payload.purchaseChannel || '').toLowerCase();
  const payment = String(payload.paymentType || '').toLowerCase();
  const age = Number(payload.age || 0);

  let predictedProfile = 'Cliente Econômico';
  let riskLevel = 'medio';
